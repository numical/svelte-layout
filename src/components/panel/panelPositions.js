...
const resize = (state, w) => ({
  ...
  graphWidth: state.overlayPanel
    ? 100
    : state.restorePanelPosition.calcGraphWidth(w),
  panelWidth: state.restorePanelPosition.calcPanelWidth(w),
  restorePanelWidth: state.restorePanelPosition.calcRestorePanelWidth(
    w,
    state.restorePanelWidth
  ),
  transition: 0,
});
